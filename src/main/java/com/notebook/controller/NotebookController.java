...
public class NotebookController {
    ...
    public String listTrash(Model model) {
        String username = getUsername();
        List<Notebook> trashNotes = notebookService.findTrashedNotebooksByUser(username);
        model.addAttribute("notebooks", trashNotes);
        model.addAttribute("username", username);
        return "notebook-trash";
    }

    /** Restore from trash */
    @GetMapping("/restore/{id}")
    public String restoreNotebook(@PathVariable("id") Long id) {
        Notebook notebook = notebookService.getNotebookById(id);
        String username = getUsername();
        if (notebook != null && notebook.getUser().getUsername().equals(username)) {
            notebookService.restoreNotebook(id);
        }
        return "redirect:/notebook/trash?restored";
    }

    /** Permanent delete */
    @GetMapping("/force-delete/{id}")
    public String forceDeleteNotebook(@PathVariable("id") Long id) {
        Notebook notebook = notebookService.getNotebookById(id);
        String username = getUsername();
        if (notebook != null && notebook.getUser().getUsername().equals(username)) {
            notebookService.forceDeleteNotebook(id);
        }
        return "redirect:/notebook/trash?deleted";
    }

    /** Auto-save (AJAX) */
    @PostMapping("/autosave")
    @ResponseBody
    public ResponseEntity<Map<String, Object>> autoSave(@RequestBody Map<String, String> payload) {
        try {
            String title = payload.get("title");
            String content = payload.get("content");
            String idStr = payload.get("id");
            if (idStr == null || idStr.isBlank()) return ResponseEntity.badRequest().build();
            
            Long id = Long.valueOf(idStr);
            Notebook nb = notebookService.autoSaveDraft(id, title, content, getUsername());
            if (nb != null) {
