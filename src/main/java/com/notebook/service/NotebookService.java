...
public class NotebookService {
    ...
    public List<Notebook> findNotebooksByUser(String username) {
        User user = userRepository.findByUsername(username).orElse(null);
        return user != null
                ? notebookRepository.findByUserAndIsDeletedFalseOrderByPinnedDescUpdatedAtDesc(user)
                : List.of();
    }

    public List<Notebook> findTrashedNotebooksByUser(String username) {
        User user = userRepository.findByUsername(username).orElse(null);
        return user != null
                ? notebookRepository.findByUserAndIsDeletedTrueOrderByUpdatedAtDesc(user)
                : List.of();
    }
    ...
    public boolean deleteCategoryFromUser(Long categoryId, String username) {
        ...
        // Lấy tất cả notebook của user (cần bao gồm cả deleted để xóa hoàn toàn liên kết? Có)
        List<Notebook> userNotebooks = notebookRepository.findByUserAndIsDeletedFalseOrderByUpdatedAtDesc(user);
    ...
    @Transactional
    public void restoreNotebook(Long id) {
        Notebook notebook = notebookRepository.findById(id).orElse(null);
    ...
    @Transactional
    public void forceDeleteNotebook(Long id) {
        notebookRepository.deleteById(id);
    }

    @Transactional
    public Notebook autoSaveDraft(Long id, String title, String content, String username) {
        if (id == null) return null; // Only auto-save existing edits
        Notebook notebook = notebookRepository.findById(id).orElse(null);
        if (notebook == null || !notebook.getUser().getUsername().equals(username)) return null;
